...
| 8 | **Remove Focus Timer (Pomodoro)** | Not started | Delete the Pomodoro widget, its ambient-sound playback, the "Start Pomodoro Focus" task-card button, and the Dashboard "Focus Streak" stat — full footprint detailed below. Task-level time tracking (`timeLogged`/`timeSessions`, Phase 5/7's actual data source) is untouched. | Delete `src/components/Pomodoro/Pomodoro.jsx`; edit [App.jsx](src/App.jsx) (unmount + `pomodoroSessions` default), [Dashboard.jsx](src/components/Dashboard/Dashboard.jsx) (Focus Streak card + calc), [ProjectDetail.jsx](src/components/ProjectDetail/ProjectDetail.jsx) (task-card button + custom event), [audio.js](src/utils/audio.js) (remove ambient-sound exports only, keep `playCompletionChime`), `server.js`/`server/db.js` (stop writing `pomodoro_sessions_json`, leave the column in place rather than a risky `DROP COLUMN` migration) | none |
| 9 | **JWT-based authentication with cookie delivery** | Not started | Replace the current opaque-token-in-DB session model with signed JWTs, still delivered via the same httpOnly cookie mechanism. Recommended design: short-lived signed access-token JWT (~15-30 min) for fast per-request verification, plus a longer-lived server-side-revocable refresh token (reusing the shape of today's `sessions` table) in a second httpOnly cookie, exchanged via a new refresh endpoint. Design rationale and trade-offs detailed below. | `server.js` (auth routes + `authenticate` middleware rewrite), `server/db.js` (rename/repurpose `sessions` → `refresh_tokens`), new dependency `jsonwebtoken`, new `JWT_SECRET` env var | Phase 1 |
| 10 | **Admin Console UI** | Not started (recommended) | A frontend page for what's currently API/Postman-only: provisioning users, assigning job titles, managing manager↔employee links, deactivating accounts. Confirmed via codebase search: no admin UI exists today — `GET/POST /api/admin/users`, `PUT /api/admin/users/:id`, `POST/DELETE /api/admin/managers` are all live and correctly `requireAdmin`-gated, but an admin has no in-app way to use them. | New `src/components/Admin/AdminConsole.jsx` + route, [App.jsx](src/App.jsx) (nav link, admin-only) | Phase 1 |
| 11 | **Automated test suite** | Not started (recommended) | Unit + integration tests. Confirmed via codebase search: zero test files or test script exist anywhere in the project today — every permission/visibility bug found across Phases 1-7 (the project-visibility leak, the manager-reports cross-boundary leak, the missing member-hierarchy check, etc.) was caught by manual live testing, not by anything that runs automatically. Highest-value target: an integration suite around the task/project visibility predicate and the manager-hierarchy rules, since that's where every real bug in this build has lived. | New `package.json` test script + framework (e.g. Vitest for frontend, supertest-style for `server.js` routes), new `tests/` directory | none, but most valuable once run against Phases 1-9 |
| 12 | **Close remaining visibility/privacy gaps** | Not started (recommended) | Bundles three already-identified, still-open items rather than one-off patches: (a) `GET /api/users/all` returns the full company directory to any authenticated user with no scoping — narrow it or confirm it's intended; (b) `GET /api/kpi/dashboard`'s `memberWorkloadList`/`projectHealthList` now expose every project's name and every person's task/time totals org-wide as a side effect of the Phase 7 scoping fix — decide whether to anonymize or accept; (c) bulk task operations still don't fire per-task `task_assigned`/`task_completed` notifications, only the project-completion check added in Phase 6. | `server.js` (all three routes) | Phases 6, 7 |
| 13 | **Real-time notification delivery** | Not started (recommended) | Upgrade from the current 15s polling to push-based delivery (WebSocket or SSE), plus native browser notifications (the Notification API — currently unused anywhere in the app) for users with the tab backgrounded, plus per-user notification-type preferences (e.g. mute `task_assigned`). The plan originally chose polling deliberately for simplicity (see Context decisions); this is an explicit upgrade proposal, not a bug fix. | `server.js` (WebSocket/SSE server), [NotificationBell.jsx](src/components/Notifications/NotificationBell.jsx), new preferences table/route | Phase 6 |
